...
def search_collection_pids(client: httpx.Client, collection_pid: str) -> list[dict]:
    ...
    while True:
        url = f'{SEARCH_URL}?q=*:*&fq={httpx.QueryParams({"fq": fq})["fq"]}&fl={fl}&rows={rows}&start={start}'
        resp = _retrying_get(client, url)
        data = resp.json()
...
def fetch_item_json(client: httpx.Client, pid: str) -> dict:
    ...
    """
    url = ITEM_URL_TPL.format(pid=pid)
    resp = _retrying_get(client, url)
    resp.raise_for_status()
    return resp.json()
...
def _extract_child_pids(item_json: dict) -> list[str]:
    ...
    """
    rels = item_json.get('relations', {})
    has_part = rels.get('hasPart', [])
    child_pids: list[str] = []
...
def _extract_size_from_datastreams(item_json: dict) -> int | None:
    ...
    """
    ds_block = item_json.get('datastreams', {}) or {}
    entry = ds_block.get('EXTRACTED_TEXT')
...
def add_listing_entry(listing: dict, *, item_pid: str, primary_title: str, full_item_api_url: str, full_studio_url: str, extracted_text_file_size: int | None) -> None:
    ...
    idx = next((i for i, d in enumerate(items) if d.get('item_pid') == item_pid), None)
    entry = {
        'item_pid': item_pid,
...
def process_pid_for_extracted_text(client: httpx.Client, pid: str, out_txt_path: Path, listing: dict) -> bool:
    ...
    """
    item_json = fetch_item_json(client, pid)
    primary_title = item_json.get('primary_title') or item_json.get('mods_title_full_primary_tsi') or ''
    studio_url = item_json.get('uri') or f'{BASE}/studio/item/{pid}/'
    item_api_url = ITEM_URL_TPL.format(pid=pid)

    found = _find_extracted_text_link_and_size(item_json, pid)
    if found:
        url, size = found
    ...
    child_pids = _extract_child_pids(item_json)
    for child_pid in child_pids:
        child_json = fetch_item_json(client, child_pid)
        child_title = child_json.get('primary_title') or child_json.get('mods_title_full_primary_tsi') or ''
        child_studio_url = child_json.get('uri') or f'{BASE}/studio/item/{child_pid}/'
        child_api_url = ITEM_URL_TPL.format(pid=child_pid)
        child_found = _find_extracted_text_link_and_size(child_json, child_pid)
        if child_found:
...
def main() -> int:
    ...
    processed = already_processed(listing)
        ...
        for i, doc in enumerate(tqdm(docs, total=len(docs), desc="Processing items"), start=1):
            ...
            try:
                ...
                add_listing_entry(
                    listing,
                    item_pid=pid,
                    primary_title=doc.get('primary_title') or '',
